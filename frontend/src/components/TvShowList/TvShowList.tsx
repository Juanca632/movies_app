...
function TvShowList({ endpoint, items, title, person }: MovieListProps) {
  ...
  }, [error, endpoint]); // Dependency is the error, so the effect reruns when an error occurs
  ...
  return (
    <div className="">
      <h1 className="text-white xl:text-3xl text-2xl font-bold mb-3 xl:!px-10 !px-5">{title}</h1>
      {person ? (
        <Swiper
          slidesPerView={'auto'}
          spaceBetween={10}
